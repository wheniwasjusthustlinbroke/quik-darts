              ...
              <div className="topbar__profile-info">
                ...
                {(!user || isAnonymous) && (
                  <div className="topbar__auth">
                    <p className="home__auth-prompt home__auth-prompt--compact">
                      {isAnonymous ? 'Sign in to save progress' : 'Sign in to play online & save progress'}
                    </p>
                    <div className="home__auth-buttons home__auth-buttons--compact">
                      <button type="button" className="home__auth-btn home__auth-btn--google home__auth-btn--icon-only" onClick={signInWithGoogle} disabled={authLoading} aria-label="Sign in with Google" title="Sign in with Google">
                      ...
                      <button type="button" className="home__auth-btn home__auth-btn--facebook home__auth-btn--icon-only" onClick={signInWithFacebook} disabled={authLoading} aria-label="Sign in with Facebook" title="Sign in with Facebook">
                        <svg viewBox="0 0 24 24" width="18" height="18" fill="#1877F2" aria-hidden="true" focusable="false">
                          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                        </svg>
                        <span className="sr-only">Sign in with Facebook</span>
                      </button>
                      <button type="button" className="home__auth-btn home__auth-btn--apple home__auth-btn--icon-only" onClick={signInWithApple} disabled={authLoading} aria-label="Sign in with Apple" title="Sign in with Apple">
