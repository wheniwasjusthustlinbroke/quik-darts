...
import { SETTLEMENT_LOCK_TIMEOUT_MS, CREATE_GAME_LOCK_TIMEOUT_MS } from '../wagering/settlementConstants';
import { FULFILLMENT_LOCK_TIMEOUT_MS } from '../payments/paymentConstants';
...
const AD_REWARD_LOCK_TIMEOUT_MS = 120_000;
...
interface StuckRecord {
  ...
  ageMs: number;
  owner?: string;
  anomaly?: boolean; // True if timestamp missing/invalid (worst case - will never self-recover)
}
...
interface ReconciliationReport {
  ...
  stuckSettling: StuckRecord[];
  stuckRefunding: StuckRecord[];
  stuckCreateGame: StuckRecord[];
  stuckFulfillments: StuckRecord[];
  stuckAdRewards: StuckRecord[];
  totalStuck: number;
 ...
 */
export const reconcileStuckStates = functions
  .region('europe-west1')
  ...
  .onRun(async () => {
    ...
    const stuckSettling = await safeDetect(
      () => findStuckByTimestamp(
        'escrow',
        'settlementStartedAt',
        'status',
        'settling',
        SETTLEMENT_LOCK_TIMEOUT_MS + STALE_BUFFER_MS,
        now
      ),
      'settling',
      errors
    ...
    const stuckCreateGame = await safeDetect(
      () => findStuckByTimestamp(
        'escrow',
        'createGameStartedAt',
        'createGameStatus',
        'processing',
        CREATE_GAME_LOCK_TIMEOUT_MS + STALE_BUFFER_MS,
        now
      ),
      'createGame',
      errors
    ...
    const stuckRefunding = await safeDetect(
      () => findStuckRefundingHybrid(REFUND_STALE_MS + STALE_BUFFER_MS, now),
      'refunding',
      errors
    );

    // Fulfillments: status-first (terminal records keep timestamps)
    const stuckFulfillments = await safeDetect(
      () => findStuckByStatus(
        'stripeFulfillments',
        'status',
        'processing',
        'startedAt',
        FULFILLMENT_LOCK_TIMEOUT_MS + STALE_BUFFER_MS,
        now
      ),
      'fulfillments',
      errors
    ...
    const stuckAdRewards = await safeDetect(
      () => findStuckByStatus(
        'verifiedAdRewards',
        'status',
        'processing',
        'processingStartedAt',
        AD_REWARD_LOCK_TIMEOUT_MS + STALE_BUFFER_MS,
        now
      ),
      'adRewards',
      errors
    ...
    const report: ReconciliationReport = {
      ...
      errors,
      thresholds: {
        settlement: SETTLEMENT_LOCK_TIMEOUT_MS,
        refund: REFUND_STALE_MS,
        createGame: CREATE_GAME_LOCK_TIMEOUT_MS,
        fulfillment: FULFILLMENT_LOCK_TIMEOUT_MS,
    ...
    if (totalStuck > 0) {
      console.error(
        `[reconcileStuckStates] ALERT: ${totalStuck} stuck records found (${totalAnomalies} anomalies). ` +
 ...
 */
async function findStuckRefundingHybrid(
  staleThresholdMs: number,
  now: number
): Promise<DetectionResult> {
  // 1. Timestamp-first: find stale refunding with valid timestamps
  const staleResult = await findStuckByTimestamp(
    'escrow',
    'refundStartedAt',
    'status',
    'refunding',
  ...
  const anomalyResult = await findMissingTimestampAnomalies(
    'escrow',
    'status',
    'refunding',
    'refundStartedAt'
 ...
 */
async function findStuckByTimestamp(
  nodePath: string,
  ...
  for (const [id, entry] of Object.entries(data as Record<string, any>)) {
 ...
 */
async function findMissingTimestampAnomalies(
  nodePath: string,
  statusField: string,
  targetStatus: string,
  timestampField: string
 ...
 */
async function findStuckByStatus(
  nodePath: string,
  statusField: string,
  targetStatus: string,
  timestampField: string,
 ...
 */
function logSampleIds(type: string, records: StuckRecord[]): void {
 ...
 */
async function pruneOldReports(now: number): Promise<void> {
