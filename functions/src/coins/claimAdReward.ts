...
const AD_REWARD = 25 as const;

// Max ads per day
const MAX_ADS_PER_DAY = 5 as const;

// Milliseconds in a day
const DAY_MS = 24 * 60 * 60 * 1000;

// Stale claim locks can be retried after this duration (2 minutes)
const AD_REWARD_LOCK_TIMEOUT_MS = 120_000 as const;
...
export const claimAdReward = functions
  ...
  .https.onCall(async (data: AdRewardRequest, context): Promise<ClaimResult> => {
    ...
    const requestId = `ad_${userId}_${randomUUID()}`;
    ...
    const claimResult = await verifiedRewardRef.transaction((reward) => {
      ...
      if (reward == null || typeof reward !== 'object') return; // Abort - missing/invalid

      // Verify the reward belongs to this user
      if (reward.userId !== userId) return; // Abort - wrong user (don't expose via error)

      // Check if reward is expired (24 hours max)
      if (now - (reward.verifiedAt || 0) > MAX_REWARD_AGE_MS) return; // Abort - expired
      ...
      if (status === 'completed') return; // Abort - already done
    ...
    const walletResult = await walletRef.transaction((wallet) => {
      ...
      if (adsToday >= MAX_ADS_PER_DAY) return; // Abort - daily limit (no marker written)
    ...
    if (!walletResult.committed) {
      ...
      if (!markerSnap.exists()) {
        ...
        if (adsToday >= MAX_ADS_PER_DAY) {
          throw new functions.https.HttpsError(
            'resource-exhausted',
            `Daily limit reached (${MAX_ADS_PER_DAY} ads per day)`
          );
        }
