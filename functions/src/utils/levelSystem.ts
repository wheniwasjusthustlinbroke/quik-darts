...
export function getLevelUpCoins(newLevel: number): number {
  // Milestone rewards
  if (newLevel === 5) return 100;
  if (newLevel === 10) return 200;
  if (newLevel === 25) return 500;
  if (newLevel === 50) return 1000;
  if (newLevel === 75) return 2000;
  if (newLevel === 100) return 5000;

  // Regular levels: 10 coins per level
  if (newLevel % 5 === 0) return 50; // Every 5 levels
  return 10;
}

// Generate XP table for reference
export function generateXPTable(): Array<{ level: number; xpRequired: number; totalXP: number }> {
  const table = [];
