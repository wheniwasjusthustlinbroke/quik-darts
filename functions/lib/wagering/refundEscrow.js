...
async function refundSingleEscrow(escrowId, reason, options) {
    ...
    const refundResult = await escrowRef.transaction((currentEscrow) => {
        ...
        if (esc.status === 'refunding') {
            ...
            if (refundAge < REFUND_STALE_MS && esc.refundRequestId)
                return;
        }
        else {
 ...
 */
async function refundPlayer(playerId, amount, escrowId, reason, timestamp, refundRequestId) {
    ...
    try {
        walletResult = await walletRef.transaction((wallet) => {
            ...
            if (wallet.refundMarkers?.[escrowId])
                return;
