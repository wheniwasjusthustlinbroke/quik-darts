...
exports.getMissTier = getMissTier;
exports.getDirectionalBias = getDirectionalBias;
exports.getCandidates = getCandidates;
exports.selectFromCandidates = selectFromCandidates;
exports.applySegmentMiss = applySegmentMiss;
const dartboardGeometry_1 = require("./dartboardGeometry");
// =============================================================================
// Constants
// =============================================================================
const MAX_REROLLS = 5;
 ...
 */
function getMissTier(power, perfectMin, perfectMax) {
    ...
    const distanceFromPerfect = power <= perfectMin ? perfectMin - power : power - perfectMax;
    if (distanceFromPerfect <= 6)
        return 'slight';
    if (distanceFromPerfect <= 12)
        return 'medium';
    if (distanceFromPerfect <= 22)
        return 'wide';
    return 'scatter';
}
/**
 ...
 * Uses strict bounds (> / <) to match isInPerfectZone in legacyMeter.
 */
function getDirectionalBias(power, perfectMin, perfectMax) {
    if (power > perfectMin && power < perfectMax)
        return 'neutral';
    if (power <= perfectMin)
        return 'weak';
 ...
 */
function getTripleCandidates(aimed, tier, bias) {
    const candidates = [];
    const num = aimed.number;
    const [left1, right1] = (0, dartboardGeometry_1.getAngularNeighbors)(num, 1);
    const [left2, right2] = (0, dartboardGeometry_1.getAngularNeighbors)(num, 2);
    ...
    if (tier === 'slight') {
        ...
        const angularNeighbors = (0, dartboardGeometry_1.getAllAngularNeighbors)(num, 3);
        ...
        const angularNeighbors = (0, dartboardGeometry_1.getAllAngularNeighbors)(num, 4);
        for (const { number: n, distance } of angularNeighbors) {
 ...
 */
function getDoubleCandidates(aimed, tier, bias) {
    const candidates = [];
    const num = aimed.number;
    const [left1, right1] = (0, dartboardGeometry_1.getAngularNeighbors)(num, 1);
    const [left2, right2] = (0, dartboardGeometry_1.getAngularNeighbors)(num, 2);
 ...
 */
function getSingleCandidates(aimed, tier, bias) {
    const candidates = [];
    const num = aimed.number;
 ...
 */
function getBullCandidates(tier, _bias) {
    const candidates = [];
    if (tier === 'slight') {
        ...
        for (const num of dartboardGeometry_1.NUMBER_ORDER) {
 ...
 */
function getOuterBullCandidates(tier, _bias) {
    const candidates = [];
    if (tier === 'slight') {
 ...
 */
function getCandidates(aimed, tier, bias) {
    ...
    return candidates.filter((c) => !(0, dartboardGeometry_1.segmentsEqual)(c.segment, aimed) && c.weight > 0);
}
/**
 * Select a segment from weighted candidates using RNG.
 * Deterministic given same RNG sequence.
 */
function selectFromCandidates(candidates, rng) {
 ...
 */
function applySegmentMiss(aimX, aimY, power, perfectMin, perfectMax, rng) {
    const aimed = (0, dartboardGeometry_1.segmentFromPosition)(aimX, aimY);
    const tier = getMissTier(power, perfectMin, perfectMax);
    // Guard: if somehow aiming outside board, return MISS
    if (aimed.ring === 'MISS') {
        const point = (0, dartboardGeometry_1.randomPointInSegment)(aimed, rng);
        return {
            x: point.x,
            y: point.y,
            segment: aimed,
    ...
    if (tier === 'perfect') {
        ...
        return {
            ...
            y: point.y,
            segment: aimed,
            label: (0, dartboardGeometry_1.getSegmentLabel)(aimed),
            tier,
        };
    }
    ...
    let point = (0, dartboardGeometry_1.randomPointInSegment)(selected, rng);
    let landed = (0, dartboardGeometry_1.segmentFromPosition)(point.x, point.y);
    // V1 RULE: Re-roll if landed segment equals aimed (belt-and-suspenders)
    let retries = 0;
    while ((0, dartboardGeometry_1.segmentsEqual)(landed, aimed) && retries < MAX_REROLLS) {
        selected = selectFromCandidates(candidates, rng);
        point = (0, dartboardGeometry_1.randomPointInSegment)(selected, rng);
        landed = (0, dartboardGeometry_1.segmentFromPosition)(point.x, point.y);
        retries++;
    }
    // Final fallback: if still aimed, force MISS
    if ((0, dartboardGeometry_1.segmentsEqual)(landed, aimed)) {
        point = (0, dartboardGeometry_1.randomPointInSegment)({ ring: 'MISS' }, rng);
        landed = { ring: 'MISS' };
    }
    return {
        x: point.x,
        y: point.y,
        segment: landed,
        label: (0, dartboardGeometry_1.getSegmentLabel)(landed),
        tier,
    };
}
//# sourceMappingURL=segmentMiss.js.map